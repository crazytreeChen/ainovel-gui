...
class AppDatabase {
    ...
    migrate() {
        ...
        // 迁移：添加 tags 列（如果不存在）
        try {
            this.database.exec('ALTER TABLE books ADD COLUMN tags TEXT DEFAULT ""');
        }
        catch { }
    }
    listBooks() {
    ...
    `).all();
        return rows.map(r => {
            const completed = (() => { try {
                return JSON.parse(r.completed_chapters_json || '[]');
            }
            catch {
                return [];
            } })();
    ...
    deleteBook(id) {
        ...
        const del = this.database.transaction(() => {
            for (const t of tables)
                this.database.prepare(`DELETE FROM ${t} WHERE book_id = ?`).run(id);
    ...
    saveUserRules(bookId, rules) {
        this.database.prepare(`INSERT OR REPLACE INTO user_rules (book_id, version, status, structured, preferences, sources, uncertain) VALUES (?,?,?,?,?,?,?)`)
            .run(bookId, rules.version || 1, rules.status || 'ready', JSON.stringify(rules.structured || {}), rules.preferences || '', JSON.stringify(rules.sources || []), JSON.stringify(rules.uncertain || []));
    }
    ...
    saveVolumes(bookId, volumes) {
        ...
        const tx = this.database.transaction(() => {
            ...
            for (const v of volumes || [])
                ins.run(bookId, v.index || v.idx || 0, v.title || '', v.theme || '');
        });
        tx();
    }
    getVolumes(bookId) {
    ...
    saveArcs(bookId, arcs) {
        ...
        const tx = this.database.transaction(() => {
            del.run(bookId);
            for (const a of arcs || [])
                ins.run(bookId, a.volume_idx || a.volumeIdx || 0, a.idx || a.index || 0, a.title || '', a.goal || '', a.estimated_chapters || a.estimatedChapters || 0, a.expanded ? 1 : 0);
        });
        tx();
    }
    getArcs(bookId) {
    ...
    saveCompass(bookId, compass) {
        if (!compass) {
            this.database.prepare('DELETE FROM compass WHERE book_id = ?').run(bookId);
            return;
        }
    ...
    saveTimelineEvents(bookId, events) {
        ...
        const tx = this.database.transaction(() => {
            ...
            for (const e of events || [])
                ins.run(bookId, e.chapter || 0, e.time || '', e.event || '', JSON.stringify(e.characters || []));
        });
        tx();
    }
    ...
    saveForeshadowEntries(bookId, entries) {
        ...
        const tx = this.database.transaction(() => {
            del.run(bookId);
            for (const e of entries || [])
                ins.run(e.id || '', bookId, e.description || '', e.plantedAt || e.planted_at || 0, e.status || 'planted', e.resolvedAt || e.resolved_at || 0);
        });
        tx();
    }
    ...
    saveRelationshipEntries(bookId, entries) {
        ...
        const tx = this.database.transaction(() => {
            del.run(bookId);
            for (const e of entries || [])
                ins.run(bookId, e.characterA || e.character_a || '', e.characterB || e.character_b || '', e.relation || '', e.chapter || 0);
        });
        tx();
    }
    ...
    saveStateChanges(bookId, changes) {
        ...
        const tx = this.database.transaction(() => {
            ...
            for (const c of changes || [])
                ins.run(bookId, c.chapter || 0, c.entity || '', c.field || '', c.oldValue || c.old_value || '', c.newValue || c.new_value || '', c.reason || '');
        });
        tx();
    }
    ...
    saveWorldRules(bookId, rules) {
        ...
        const tx = this.database.transaction(() => {
            ...
            for (const r of rules || [])
                ins.run(bookId, r.category || '', r.ruleText || r.rule_text || '', r.boundary || '');
        });
        tx();
    }
    ...
    updateBook(id, fields) {
        ...
        if (fields.style !== undefined) {
            sets.push('style = ?');
            params.push(fields.style);
        }
        if (fields.premise !== undefined) {
            sets.push('premise = ?');
            params.push(fields.premise);
        }
