...
function getBookDirById(id) {
  ...
  } catch (e) { log.error('getBookDirById', e) }
  return join(GUI_DATA_DIR, 'books', id)
}

function readStoreJSONAt(dir, rel) {
  const f = join(dir, rel)
  if (!existsSync(f)) return null
  try { return JSON.parse(readFileSync(f, 'utf8')) } catch { return null }
}

function readStoreTextAt(dir, rel) {
  const f = join(dir, rel)
  if (!existsSync(f)) return null
...
function register(ipcMain) {
  ...
  ipcMain.handle('get-book-outline', async (_e, id) => {
    ...
    } catch (e) { log.error('get-book-outline:sqlite', e) }
    if (!outline?.length) {
      outline = readStoreJSONAt(dir, 'outline.json')
      try { if (outline?.length) db.saveOutlineEntries(id, outline) } catch (e) { log.error('get-outline:save', e) }
    }
    if (!layeredOutline?.length) layeredOutline = readStoreJSONAt(dir, 'layered_outline.json')
    if (!compass) {
      compass = readStoreJSONAt(dir, 'compass.json')
      try { if (compass) db.saveCompass(id, compass) } catch (e) { log.error('get-outline:save-compass', e) }
    }
  ...
  ipcMain.handle('get-book-chapters', async (_e, id) => {
    ...
    try {
      ...
      if (dbCh?.length) return dbCh.map(c => ({ num: c.num, title: c.title || `第${c.num}章`, wordCount: c.word_count || 0, status: c.status || 'completed' }))
    } catch (e) { log.error('get-book-chapters:sqlite', e) }
    const chDir = join(dir, 'chapters')
    if (!existsSync(chDir)) return []
    return readdirSync(chDir).filter(f => f.endsWith('.md')).sort().map(file => {
      const num = parseInt(file.replace('.md', ''), 10)
      if (isNaN(num)) return null
      try {
        const content = readFileSync(join(chDir, file), 'utf8')
        const title = content.split('\n')[0]?.replace(/^#\s*/, '').trim() || `第${num}章`
        try { getDB().saveChapter(id, num, content, title) } catch (e) { log.error('get-chapters:sync', e) }
        return { num, title, wordCount: content.length, status: 'completed' }
      } catch (e) { log.warn('get-chapters:read', e?.message || e); return { num, title: `第${num}章`, wordCount: 0, status: 'completed' } }
    }).filter(Boolean)
  })
  ...
  ipcMain.handle('get-book-chapter', async (_e, id, num) => {
    ...
    } catch (e) { log.error('get-book-chapter:sqlite', e) }
    ...
    if (existsSync(planFile)) { try { plan = JSON.parse(readFileSync(planFile, 'utf8')) } catch (e) { log.error('get-chapter:plan', e) } }
    try {
    ...
    } catch (e) { log.error('get-chapter:sync', e) }
  ...
  ipcMain.handle('get-book-characters', async (_e, id) => {
    ...
    } catch (e) { log.error('get-characters', e) }
    const chars = readStoreJSONAt(dir, 'characters.json') || []
    try { if (chars.length) getDB().saveCharacters(id, chars) } catch (e) { log.error('get-characters:sync', e) }
    return chars
  })
  ...
  ipcMain.handle('get-book-timeline', async (_e, id) => {
    ...
    } catch (e) { log.error('get-timeline:sqlite', e) }
    ...
    } catch (e) { log.error('get-timeline:sync', e) }
  ...
  ipcMain.handle('get-book-reviews', async (_e, id) => {
    ...
    } catch (e) { log.error('get-reviews:sqlite', e) }
    ...
    try { if (reviews.length) getDB().saveReviews(id, reviews) } catch (e) { log.error('get-reviews:sync', e) }
    return reviews
  })
  ...
  ipcMain.handle('get-simulation-profile', async (_e, bookId) => {
    try {
      ...
      if (row) return row
    } catch (e) { log.error('get-sim-profile', e) }
    const dir = getBookDirById(bookId)
    if (!dir) return null
    const profile = readStoreJSONAt(dir, 'simulation_profile.json')
    if (profile) { try { getDB().saveSimulationProfile(bookId, profile) } catch (e) { log.error('get-sim-profile:sync', e) }; return profile }
    return null
  })
