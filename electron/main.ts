...
function findActiveBookDir() {
  if (!outputDir) return null
  const outputSub = join(outputDir, 'output')
  if (!existsSync(outputSub)) return outputDir // 降级到 outputDir 本身
  try {
    const entries = readdirSync(outputSub, { withFileTypes: true })
    for (const entry of entries) {
      if (entry.isDirectory()) {
    ...
    if (isAlive) {
      ...
      try {
        ...
        if (books && books.length > 0) {
          ...
          if (fullBook?.premise) snap.premise = fullBook.premise.slice(0, 200)
          // 读取角色
          try {
          ...
          try {
            ...
            if (entries && entries.length > 0) {
              snap.outline = entries.slice(-30).map((e: any) => ({
                chapter: e.chapter || 0,
                title: e.title || '',
    ...
    try {
      ...
      if (books && books.length > 0) {
        ...
        try {
          const usage = getDB().getUsageStats(bookId)
          if (usage) {
            if (!snap.totalInputTokens) snap.totalInputTokens = usage.total_input || 0
            if (!snap.totalOutputTokens) snap.totalOutputTokens = usage.total_output || 0
            if (!snap.totalCostUSD) snap.totalCostUSD = usage.total_cost || 0
            if (!snap.totalSavedUSD) snap.totalSavedUSD = usage.total_saved || 0
  ...
  ipcMain.handle('resume-writing', async (_e, bookId) => {
    ...
    try {
      ...
      ainovelProcess.stderr.on('data', (data) => {
        ...
        // 保留最近 2000 条
        if (engineEvents.length > 2000) engineEvents.splice(0, engineEvents.length - 2000)
      })
...
function startRuntimeSync() {
  ...
  syncTimer = setInterval(() => {
    if (!ainovelProcess || ainovelProcess.exitCode !== null) return
    try {
      const progress = readStoreJSON('meta/progress.json')
      if (!progress) return
      // 查找当前书籍 ID
      ...
      getDB().database.prepare(`
        ...
        WHERE book_id=?
      `).run(
        progress.novel_name || '', progress.phase || '', progress.current_chapter || 0,
        progress.total_chapters || 0, JSON.stringify(completed), totalWords,
        JSON.stringify(wordCounts), progress.in_progress_chapter || 0,
        progress.flow || '', JSON.stringify(progress.pending_rewrites || []),
        progress.rewrite_reason || '', progress.current_volume || 0,
        progress.current_arc || 0, bookId
      )
      // 更新 books 表字数
